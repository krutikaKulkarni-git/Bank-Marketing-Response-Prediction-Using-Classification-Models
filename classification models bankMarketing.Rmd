---
title: "Bank Marketing Case...sample code"
author: "sid b"
...
IDS 572 -  Data exploration, 
           Decision trees and other models,  classification performance evaluation
...
setwd("/Users/krutikakulkarni/Downloads")
list.files()
bankData=read.csv2('bank-full.csv') #read_csv2 is used since the data has ';' as delimiter
...
str(bData)
   # do you see the converted variables?  Is this what you want?
#We see the variables converted to categorical variables 
...
#The frequency changes as the grouping changes, since frequency is calculated relative to the total number of rows for first variable.
...
#The subscription is changing according to age and most of them lie between 20 and 75, above 75 its mostly ouliers
...
bData %>% group_by(ageGroup, y) %>% tally() %>% view()
bData %>% group_by(ageGroup, y) %>% tally() %>% mutate(propResp=n/sum(n)) %>% view()

#can plot the response rate by age group
tmp <-bData %>% group_by(ageGroup, y) %>% tally() %>% mutate(propResp=n/sum(n)) %>% view()
...
#the response is comparatively lower
...
ggplot(bData, aes(duration,  color=y) ) + geom_density()
    ...
    #     left-skewed distribution -- most calls are relatively short, and some a overly long and can be excluded from the analysis

#Look also at number of calls (campaign)
summary(bData$campaign)
ggplot(bData, aes(campaign,  color=y) ) + geom_boxplot()
  #what do you observe?  - most customers have ben contacted 1-3 times;a few have very been contacted a very large number times (max 63 !)
...
ggplot(bData, aes(duration, campaign, color=y))+geom_point()
   #What do you observe?
   #   - most responders (y=yes) were contacted less often and had longer duration calls and responders (y=no)d had short duration of call 
...
# Split the data into training and test sets – what proportion do you use for training? 70%
nr=nrow(mData)
